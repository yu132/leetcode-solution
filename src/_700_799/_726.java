...
public class _726 {
	 ...
	 * 乘上倍数然后在和外层的相加就可以了，比起把每个括号里面的递归一次要快
	 * 
	 * @author 87663
	 *
	 */
	...
	class Solution {
		...
		public Map<String, Integer> countDeep(String formula, int times) {
			...
			for (int i = 0; i < formula.length(); ++i) {
				...
				if (complexFormula && !complexFormulaEnd) {//复杂表达式内部

					++complexFormulaLength;//长度加一

					if (c == '(')//左括号
						++leftParenthesesCount;//左括号计数加一
					else if (c == ')') {//右括号
						++rightParenthesesCount;//右括号计数加一
						if (leftParenthesesCount == rightParenthesesCount)//左右括号数量相等，
							complexFormulaEnd = true;				//证明一个复杂表达式已经结束了
					}
				} else {//非复杂表达式
